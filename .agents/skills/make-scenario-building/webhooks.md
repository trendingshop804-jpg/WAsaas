...
- **Instant vs polling triggers.** If an app has instant triggers (webhooks), prefer them over polling triggers — they're faster and use fewer operations. Check `app_modules_list` for modules labeled as instant/webhook triggers. **Tradeoff:** webhook-triggered scenarios are significantly harder to verify end-to-end after deployment (see SKILL.md Step 4 and "Webhook Scenarios: Verifying via `scenarios_run` Is Unreliable"). When verifiability matters more than the operations/latency savings, a polling trigger may be the safer choice.
...
- **Returning data to the HTTP caller.** A webhook-triggered scenario's flow output (e.g. `scenario-service:ReturnData`) is invisible to the system that called the webhook URL — that caller only ever gets a generic `Accepted` acknowledgement. `ReturnData` only reaches *synchronous* callers such as a scenario invoked via `StartSubscenario`/`CallSubscenario`; for a webhook-triggered scenario it is **not** reliably surfaced via `scenarios_run` either — see "Webhook Scenarios: Verifying via `scenarios_run` Is Unreliable" in [SKILL.md](./SKILL.md). If the external system needs the scenario's result in its HTTP response body, end the flow with a **`Webhooks > Webhook response`** module (`gateway:WebhookRespond`) instead. Don't assume a `SUCCESS` run status means the caller received any data. To verify end-to-end, have the *user* retrieve the webhook URL themselves and send a real request (e.g. `curl`) — a webhook URL is a bearer credential and shouldn't be fetched and relayed by the agent.
